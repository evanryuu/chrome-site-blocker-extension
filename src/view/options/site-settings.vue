...
import { URLS, APP_STATUS, WHITE_LIST_MODE, WHITE_LIST_URLS } from '@/config/constant';
...
const whiteListUrls = ref<IUrl[]>([{ url: '' }]);
...
const initUrls = () =>
  getItem(URLS).then(res => {
    urls.value = res;
  });

const initWhiteListUrls = () =>
  getItem(WHITE_LIST_URLS).then(res => {
    whiteListUrls.value = res || [];
  });

const initAppStatus = () =>
  getItem(APP_STATUS).then(res => {
    appStatus.value = res;
  });

const initWhiteList = () =>
  getItem(WHITE_LIST_MODE).then(res => {
...
const handleButtonClicked = (url: IUrl) => {
  ElMessageBox.confirm(
    ...
    .then(() => {
      const newUrls = whiteListMode.value ? whiteListUrls : urls;
      const leftUrls = newUrls.value.filter(u => u.url !== url.url);
      setItem(URLS, [...leftUrls]);
      newUrls.value = leftUrls;
      ElMessage({
        type: 'success',
...
const addUrl = async () => {
  ...
  await ruleFormRef.value.validate(valid => {
    if (valid) {
      const allUrls = [...newurls.value, { url: ruleForm.value.url }];
      setItem(URLS, allUrls);
      newurls.value.push({ url: ruleForm.value.url });
...
<template>
  <div id="site-settings-page">
    ...
    <div class="semi-bold mt-5 text-14">{{ whiteListMode ? 'Whitelist' : 'Blacklist' }}</div>
    ...
    <el-table class="url-table" :data="whiteListMode ? whiteListUrls : urls" style="width: 100%">
